# -*- coding: utf-8 -*-
"""
Created on Tue May  9 01:40:15 2023
...
def parse_args():
    ...
    parser.add_argument('--model_dir', default='./logs/lp/rrnn_size=256-predictor-posterior-rnn_layers=2-1-n_past=2-n_future=10-lr=0.0020-g_dim=128-z_dim=64-last_frame_skip=False-beta=0.0001000', help='base directory to save logs')
...
def main():
    ...
    if args.model_dir != '':
        ...
        name = 'rrnn_size=%d-predictor-posterior-rnn_layers=%d-%d-n_past=%d-n_future=%d-lr=%.4f-g_dim=%d-z_dim=%d-last_frame_skip=%s-beta=%.7f'\
    ...
    for epoch in range(start_epoch, niter):
        ...
        progress.update(1)
        with open('./{}/train_record.txt'.format(args.log_dir), 'a') as train_record:
            train_record.write(('epoch: %02d | loss: %.5f | mse loss: %.5f | kld loss: %.5f | tfr: %.2f\n' % (epoch, epoch_loss  / args.epoch_size, epoch_mse / args.epoch_size, epoch_kld / args.epoch_size, args.tfr)))
