			...
			if (move->assign(b))
			{
				...
				if (pos.size() != 0)
				{
					for (int i = 0; i < pos.size(); i++)
					{
						...
						totol += (0.9 * two + 0.1 * four) / float(pos.size());
					}
				}
				else
				{
					totol = estimate(move->after_state());
				}
				move->set_value(move->reward() + totol);
...
int main(int argc, const char *argv[])
{
	...
	tdl.add_feature(new pattern({1, 3, 5, 8, 10}));
